...
//모델 역활을 제공하기 위한 클래스 - 요청 처리 메소드가 선언된 인터페이스를 상속받아 작성
// => 하나의 클라이언트 요청에 대해 하나의 요청 처리 클래스를 사용하여 클라이언트의 요청 처리 - Command Pattern
// => 클라이언트가 [/list.itwill]의 URL 주소로 요청한 경우 컨트롤러에 의해 동작될 요청 처리 클래스
public class ListController implements Controller {
	//요청 처리 메소드 : 클라이언트의 요청을 처리하기 위한 명령을 작성한 메소드
	// => 스레드가 이동되어 응답될 JSP 문서의 이름(ViewName)을 반환
	@Override
