...
@Controller
//@SessionAttributes : Model 객체를 사용하여 저장되는 속성값을 현재 요청 처리 클래스(Controller
//클래스)의 모든 요청 처리 메소드와 뷰에게 제공하는 어노테이션
// => Model 객체를 사용하여 저장되는 속성값을 Request Scope 속성값이 아닌 제한적인 Session Scope
//속성값으로 변환하여 제공하는 어노테이션
// => 변경 처리시 호출되는 요청 처리 메소드에서 변경할 객체를 검색하지 않고 뷰에서는 값을 
//전달하지 않도록 제한적인 Session Scope 속성값 사용
...
public class SessionController {
	...
	*/
	...
	// => 요청 처리 메소드의 매개변수에는 Command 객체로 @SessionAttributes 어노테이션에 의해
	//제공된 회원정보를 전달받아 저장하고 변경값으로 필드값만 변경해 사용 
	//요청 처리 메소드에 SessionStatus 인터페이스로 매개변수를 선언하면 SessionStatus 객체를
	//제공받아 사용 가능
