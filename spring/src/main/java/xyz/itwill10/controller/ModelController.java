...
public class ModelController {
	...
	//@ModelAttribute : 메소드에 의해 반환되는 객체(값)을 속성값으로 저장하여 현재 클래스에
	//작성된 모든 요청 처리 메소드의 뷰(JSP 문서)에게 제공하기 위한 어노테이션
	// => 요청 처리 메소드에 의해 응답될 모든 뷰에서 공통적으로 사용되는 객체(값)을 제공할 목적으로 사용
	//value 속성 : 뷰에서 속성값을 사용하기 위한 이름(속성명)을 속성값으로 설정
	// => 다른 속성이 없는 없는 경우 속성값만 설정 가능
	@ModelAttribute("now")
