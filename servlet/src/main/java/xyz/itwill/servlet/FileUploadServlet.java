...
public class FileUploadServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//주의)WAS 프로그램 실행(Start)될때 작업 디렉토리의 프로젝트 파일이 웹디렉토리의 자원으로 동기화 처리
		...
		//FileRenamePolicy 객체 : 전달파일을 업로드 처리할 때 서버 디렉토리에 전달파일과 같은 이름의 
		//파일이 이미 존재할 경우 전달파일의 이름을 변경하여 업로드 처리하기 위한 기능을 제공하는 객체
		// => 생성자 매개변수에 FileRenamePolicy 객체를 전달하지 않은 경우 서버 디렉토리에 전달파일과 
		//같은 이름의 파일이 있는 경우 전달파일로 기존 파일을 덮어씌우기(OverWrite) 하여 저장
		...
		//MultipartRequest.getFilesystemName(String name) : 멀티파트 폼데이타로 전달된
		//파일에 대한 업로드 파일의 이름을 문자열로 반환하는 메소드
