...
//스트림(Stream) : 값을 전달하기 위한 목적으로 생성된 입력클래스 또는 출력 클래스의 객체
// => 시냇물을 흐르는 모양을 모델링하여 만들어 값이 한쪽 방향으로만 전달되며 순차적으로 처리

//원시데이타 기반 스트림(Byte Stream) : 가공되지 않은 원시데이타를 전달하기 위한 입출력 스트림
// => 값을 1Byte 단위로 입력 또는 출력하기 위한 스트림
// => InputStream 클래스와 OutputStream 클래스를 최상위 클래스로 하는 자식클래스을 이용하여 생성
...
public class ByteStreamApp {
	public static void main(String[] args) throws IOException {
		...
		while(true) {
			...
			//InputStream.read() : 입력스트림에 존재하는 값(1Byte)을 읽어 반환하는 메소드
